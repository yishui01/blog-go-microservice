...
import (
	...
	"github.com/zuiqiangqishao/framework/pkg/setting/dsn"
	"net"
	"net/http"
	"os"
	"regexp"
...
func (engine *Engine) Start() error {
	...
	log.SugarWithContext(nil).Infof("khttp: start http listen addr: %s", conf.Addr)
	...
	go func() {
		if err := engine.RunServer(server, l); err != nil {
			if errors.Cause(err) == http.ErrServerClosed {
				log.SugarWithContext(nil).Info("khttp: : server closed")
				return
			}
...
func NewServer(conf *ServerConfig) *Engine {
	if conf == nil {
		if !flag.Parsed() {
			fmt.Fprint(os.Stderr, "[khttp] please call flag.Parse() before Init khttp server, some configure may not effect.\n")
		}
...
func (engine *Engine) handleContext(c *Context) {
	...
	//把req的参数赋值到md中，再把md注入到context，再对context设置超时，完美
