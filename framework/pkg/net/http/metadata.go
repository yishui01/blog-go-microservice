...
var _parser = map[string]func(string) interface{}{
	"mirror": func(mirrorStr string) interface{} {
		...
		if err != nil {
			log.SugarWithContext(nil).Warnf("khttp: failed to parse mirror: %+v", errors.Wrap(err, mirrorStr))
			return false
		}
		if !val {
			log.SugarWithContext(nil).Warnf("khttp: request mirrorStr value :%s is false", mirrorStr)
		}
		return val
	},
...
func setCaller(req *http.Request) {
	req.Header.Set(_httpHeaderUser, os.Getenv("AppID"))
}
