...
func TestDaoGetTagAll(t *testing.T) {
	Convey("tag operation", t, func() {
		...
		tag := &db.Tag{Name: strconv.FormatInt(time.Now().Unix(), 10) + "create55"}
		copyTag := &db.Tag{Name: tag.Name}
		...
		//加入文章中间关联
		art := &db.Article{Content: "tagdelete", Tags: strconv.FormatInt(tag.Id, 10)}
		artId, err := d.CreateArtMetas(c, art, nil)
		So(err, ShouldBeNil)
		So(artId, ShouldBeGreaterThan, 0)

		//***********************修改Tag*******************************/
		newTag := &db.Tag{Id: tag.Id, Name: strconv.FormatInt(time.Now().Unix(), 10) + "update88"}
