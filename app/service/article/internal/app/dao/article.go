...
//只从ES中查找文章列表
func (d *Dao) ArtMetasList(c context.Context, req *db.ArtQueryReq) ([]*db.EsArticle, int64, error) {
	res := make([]*db.EsArticle, 0)
...
//获取文章详情,不包含metas
func (d *Dao) GetArtBySn(c context.Context, sn string) (res *db.Article, err error) {
...
func (d *Dao) CreateArtMetas(c context.Context, art *db.Article, metas *db.Metas) (artId int64, err error) {
	var (
		db         = d.db
		tagNameStr = ""
		tags       = []*db.Tag{}
...
func (d *Dao) extractTagFromIdStr(tagIdStr string) ([]*db.Tag, string, error) {
...
func (d *Dao) updateRelationArtTag(artId int64, tags []*db.Tag, tx *gorm.DB) error {
	if tx == nil {
	...
	for _, tag := range tags {
		if err = tx.Table("mc_article_tag").Create(&db.ArticleTag{ArticleId: artId,
...
//DB修改文章+metas+tags
func (d *Dao) UpdateArtMetas(c context.Context, art *db.Article, metas *db.Metas) (id int64, err error) {
...
func (d *Dao) DelArtMetas(c context.Context, id int64, physical bool) (err error) {
	...
	if err = tx.Where("id = ?", id).Delete(&db.Article{}).Error; err != nil && err != gorm.ErrRecordNotFound {
		return errors.WithStack(err)
	}
...
func (d *Dao) RefreshArt(c context.Context, artId int64) error {
	...
	art.Tags = db.BuildArtTagStr(tags)
...
func (d *Dao) GetArtTagsFromDB(c context.Context, artId int64) ([]*db.ArticleTag, error) {
	res := make([]*db.ArticleTag, 0)
	err := d.db.Table("mc_article_tag").Where("article_id =?", artId).Find(&res).Error
	return res, errors.WithStack(err)
}

func (d *Dao) GetArtFromDB(c context.Context, query interface{}, args ...interface{}) (res *db.Article, err error) {
	res = new(db.Article)
...
func (d *Dao) GetMetasFromDB(c context.Context, query interface{}, args ...interface{}) (res *db.Metas, err error) {
	res = new(db.Metas)
	if err = d.db.Where(query, args...).First(&res).Error; err != nil {
		//todo... db err add metrics
		if err == gorm.ErrRecordNotFound {
