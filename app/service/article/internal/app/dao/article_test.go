...
func TestDaoupdateRelationArtTag(t *testing.T) {
	Convey("updateRelationArtTag", t, func() {
		var (
			tags = []*db.Tag{
				&db.Tag{Name: "tname1", Id: 1},
				&db.Tag{Name: "tname2", Id: 2},
				&db.Tag{Name: "tname3", Id: 3},
			}
		)
		Convey("When everything goes positive", func() {
			art := db.Article{Content: "111"}
	...
	Convey("CreateArtMetas", t, func() {
		...
		Convey("When everything goes positive", func() {
			var (
				...
				art   = &db.Article{Id: 1, Sn: "123456", Content: "666666", Tags: tagIdStr}
				metas = &db.Metas{ViewCount: 9, LaudCount: 19, CmCount: 99}
			)
...
func TestDaoGetArtBySn(t *testing.T) {
	...
	Eventually(func() bool {
		...
		if err := d.cacheQueue.Do(context.TODO(), func(c context.Context) {
			if err := d.SetCacheArt(c, &db.Article{Id: -5, Sn: ""}, 30); err == nil {
				res = true
			}
		}); err != nil {
...
func TestDaoUpdateArtMetas(t *testing.T) {
	Convey("UpdateArtMetas", t, func() {
		var (
			c     = context.Background()
			art   = &db.Article{Content: "666777"}
			metas = &db.Metas{LaudCount: 9, CmCount: 7, ViewCount: 88}
		)
		Convey("When everything goes positive", func() {
...
func TestDaoDelArtMetas(t *testing.T) {
	Convey("DelArtMetas", t, func() {
		...
		Convey("When everything goes positive", func() {
			art := db.Article{Title: "111"}
			metas := db.Metas{ViewCount: 111}
			...
			f := func() ([]*db.EsArticle, int64, error) {
				return d.ArtMetasList(context.TODO(), &db.ArtQueryReq{
...
func TestDaoRefreshArt(t *testing.T) {
	Convey("RefreshArt++GetArtTagsFromDB", t, func() {
		var (
			c     = context.Background()
			art   = &db.Article{Content: "777", Tags: sourceTagIdStr}
			metas = &db.Metas{ViewCount: 50}
		)
		...
		num := 500
		for i := 0; i < num; i++ {
			if err = d.AddMetasCount(c, res.Sn, db.ViewRedisKey); err != nil {
				t.Error(err)
			}
		}
