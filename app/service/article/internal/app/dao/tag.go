...
import (
	db2 "blog-go-microservice/app/service/article/internal/app/model/db"
	"context"
...
//查询Tag，一次性取出
func (d *Dao) GetTagAll(c context.Context) ([]*db2.Tag, error) {
...
//创建Tag
func (d *Dao) CreateTag(c context.Context, tag *db2.Tag) (int64, error) {
	if tag == nil || tag.Name == "" {
...
//更新Tag
func (d *Dao) UpdateTag(c context.Context, tag *db2.Tag) (tagId int64, err error) {
...
func (d *Dao) DeleteTag(c context.Context, tagId int64, physical bool) (err error) {
	...
	//先删除Tag表
	if err = tx.Where("id=?", tagId).Delete(db2.Tag{}).Error; err != nil {
		tx.Rollback()
	...
	//删除中间关联表
	if err = tx.Table("mc_article_tag").Where("tag_id = ?", tagId).Delete(db2.ArticleTag{}).
		Error; err != nil {
		tx.Rollback()
		return errors.WithStack(err)
	}
...
//从DB中直接获取Tag列表
func (d *Dao) GetAllTagFromDB(c context.Context, where ...interface{}) ([]*db2.Tag, error) {
	res := make([]*db2.Tag, 0)
...
//从DB中获取单个Tag
func (d *Dao) GetFirstTagFromDB(c context.Context, where ...interface{}) (*db2.Tag, error) {
	res := new(db2.Tag)
