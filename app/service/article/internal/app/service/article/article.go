...
func (s *Service) GetArtBySn(ctx context.Context, artReq *pb.ArtDetailRequest) (*pb.ArtDetailResp, error) {
	...
	if se := s.dao.AddMetasCount(ctx, artReq.Sn, db.ViewRedisKey); se != nil {
...
func AssignDetail(art *db.Article, metas *db.Metas) *pb.ArtDetailResp {
...
func AssignSave(req *pb.SaveArtReq) (*db.Article, *db.Metas) {
	art := new(db.Article)
	art.Id = req.Id
	art.Tags = req.Tags
	art.Title = req.Title
